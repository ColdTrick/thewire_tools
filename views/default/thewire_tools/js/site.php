...
elgg.thewire_tools.init = function() {
	...
	// support for the mentions plugin
	var mentions_plugin_enabled = false;
	try {
		elgg.require("elgg.mentions");
		mentions_plugin_enabled = true;
	} catch (err) {
		...
		$(this)
			...
			.autocomplete({
				...
				search: function() {
					...
					if (( term.length > 1) && ((!mentions_plugin_enabled && firstChar == "@") || firstChar == "#")) {
						return true;
					}
